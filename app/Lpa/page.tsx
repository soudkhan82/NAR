...
/* lazy-load only what we need from lpa.ts */
type LpaModule = typeof import("@/app/lib/rpc/lpa");
async function getLpa(): Promise<LpaModule> {
  return await import("@/app/lib/rpc/lpa");
}
...
export default function LpaPage() {
  ...
  // cache the module so we import once
  const modRef = useRef<LpaModule | null>(null);
  const loadModule = async () => (modRef.current ??= await getLpa());

  const loadFiltersOnce = async () => {
    const lpa = await loadModule();
    const opts = await lpa.fetchLpaFilters();

    const sr = ["__ALL__", ...toStringArray(opts.subregions)];
    setSubRegions(sr);

    // we’ll seed names from the first summary load below (keeps this lean)
  };
  ...
  const loadSummary = async () => {
    ...
    // derive Name picklist from data once (or whenever filter changes)
  ...
  /* total records ≈ sum of name counts (simple, no extra DB call) */
