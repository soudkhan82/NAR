...
interface FilterState {
  projects: string[]; // checkbox selections
  siteClass: SiteClass; // NEW
...
// soft heat background for 2G/3G/4G cells
const heat = (
  ...
  const alpha = 0.08 + (val / 100) * 0.22; // 0.08..0.30
...
export default function ANOpsPage() {
  ...
  // default last 180 days (keeps time-series bounded)
  useEffect(() => {
    setFilters((f) => {
  ...
  }, [filters.projects, filters.siteClass]); // NEW dep
  ...
  useEffect(() => {
    (async () => {
      ...
      const list = await fetchSites({
        ...
        siteClass: filters.siteClass, // NEW
        subregion: null,
  ...
  // Attempts total (if needed later)
